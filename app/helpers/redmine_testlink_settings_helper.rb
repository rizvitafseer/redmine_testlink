...
module RedmineTestlinkSettingsHelper
  ...
  def self.testlink_plugin_setting_text_field(setting, value, options = {})
    safe_join(
      [
        SettingsHelper.setting_label(setting, options),
        text_field_tag("settings[#{setting}]", value, options)
      ]
    )
  end

  def self.testlink_plugin_setting_multiselect(setting, choices, values, options = {})
